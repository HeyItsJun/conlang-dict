...
function loadTable() {
    table.innerHTML = "";
    const filteredEntries = input.value.trim() !== "" ? entries.filter(entry => (entry[selector.value] || "").toLowerCase().startsWith(input.value.trim().toLowerCase())) : entries;
    filteredEntries.sort((entry1, entry2) => entry1.word.localeCompare(entry2.word)); // orden alfabético
